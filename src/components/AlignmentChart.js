...
export default class AlignmentChart extends React.Component {
  ...
  componentDidMount() {
        ...
        d3.json('https://covidpy-rest.herokuapp.com/alignment')
          .then((res_data)=>{
            var data = [];
            ...
            loading = false;
            x.domain(all_dates);
            y.domain([0, d3.max(data.map((c,i)=>{return c.data[c.data.length - 1].y}))]);
            ...
            var toggle_align = () => {
              ...
              legends
                .transition().duration(1000)
                .text((d) => aligned?d.name + ' ('+d.offset+' días atrás de Brasil)':d.name);
            };
            ...
            var legend = svg.append("g")
                         ...
                         .data(data.map((c) => c))
                         .enter().append("g")
                         .attr("transform", (d, i) => "translate(0," + (i * 22 + height_all/32) + ")");
            ...
            var legends = legend.append("text")
                  .attr("x", 73)
                  .attr("y", 9.5)
                  .attr("dy", "0.32em")
                  .text((d) => d.name);
    ...
    render() {
      return(
        <>
          ...
          <button id="align">Alinear Curvas</button>
        </>
      )
    }
}
